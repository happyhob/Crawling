...
def findWordMean(word):
    # ChromeDriver 설정
        ...
        try:
            ...
            soup = BeautifulSoup(driver.page_source, 'html.parser')
            # 관련 단어 목록 가져오기
            word_items = soup.find_all('li', class_='mean_item', lang='ko')
            
            # 데이터 출력
            for item in word_items:
                # word = item.find('span', {'data-type': 'ore', 'data-lang': 'ko'}).text
                meaning = item.find('p', class_='mean', lang='ko').text.strip()
                print(meaning)
        finally:
            # 브라우저 닫기
            driver.quit()
